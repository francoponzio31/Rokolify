...
def check_if_track_was_recently_added_by_guests(user_data, track_uri):
    ...
    min_time_to_re_add = user_data["guest_settings"]["time_to_re_add_same_track"]   # Tiempo para volver a agregar la misma canción, expresado en segundos
    ...
    if track_uri in recently_added_tracks:
        time_since_added = (datetime.now() - recently_added_tracks[track_uri]["added_at"]).total_seconds()
        if time_since_added < min_time_to_re_add:
